...
def test_run_sia_loop_project_fixture_replay(tmp_path: Path, copy_project_sandbox: Copy):
    ...
    # Real numbers: recorded_generations/gen_{1,2,3}/target_agent.py genuinely
    # differ (THRESHOLD 0.5 -> 0.3 -> 0.25, matching improvement.md's prescribed
    # tuning), and each generation's results.json was regenerated by actually
    # executing that generation's agent against tasks/mini_classify's real
    # evaluate.py. All three thresholds land inside the dataset's clean
    # separation gap (0.20, 0.85], so real accuracy is 1.0 for every generation
    # -- a flat, honestly-computed trace, not a fabricated rising curve.
    assert metrics == [1.0, 1.0, 1.0]
...
def test_compute_variables_after_run(tmp_path: Path, copy_project_sandbox: Copy):
    ...
    assert variables["SIA_FINAL_METRIC_VALUE"] == "1.0000"

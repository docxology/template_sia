...
def test_manuscript_tokens_cover_manuscript_files(
    ...
    tokens: set[str] = set()
    for path in (project / "manuscript").glob("[0-9][0-9]_*.md"):
...
def test_metrics_table_matches_run_summary(
    ...
    run_sia_loop_project(project, live=False)
    variables = compute_variables(project)
    summary = load_run_summary(project, run_id=1)
    ...
    assert (
        variables["SIA_METRIC_DELTA"]
        == f"{float(variables['SIA_FINAL_METRIC_VALUE']) - float(variables['SIA_GEN1_METRIC_VALUE']):.4f}"
    )
...
def test_variables_json_round_trip(
    ...
    run_sia_loop_project(project, live=False)
    variables = compute_variables(project)
    saved = json.loads((project / "output" / "data" / "manuscript_variables.json").read_text())
